...
public final class ShizukuStampCommand extends Command {

    public ShizukuStampCommand(){
        this.name = "stampshizuku";
        this.help = "雫ちゃんのスタンプからランダムに選ばれるよ";
        this.aliases = new String[] {"shizuku", "雫", "雫ちゃん", "しずく", "しずくちゃん", "日野森雫", "日野森雫ちゃん"};
    }

    @Override
    protected void execute(final CommandEvent event) {
        final List<RandomShizukuStamp> values = Arrays.asList(RandomShizukuStamp.values());
        final int size = values.size();
        final Random rng = new Random();
        final RandomShizukuStamp shizuku = values.get(rng.nextInt(size));
        try {
            EmbedBuilder embedBuilder = new EmbedBuilder();
            final String url = "https://sekai-res.dnaroma.eu/file/sekai-assets/stamp/" + shizuku + "_rip/" + shizuku + "/" + shizuku + ".png";
            //final String url = "https://sekai-res.dnaroma.eu/file/sekai-assets/stamp/" + shizuku + "_rip/" + shizuku + "/" + shizuku + ".webp";
            embedBuilder.setImage(url);
            embedBuilder.setColor(ColorUtils.SHIZUKU.getColor());
    ...
    public enum RandomShizukuStamp {
        //応援するわ
        stamp0508(null, 0),
        ...
        RandomShizukuStamp(String url, int rng) {
            this.url = url;
            this.rng = rng;
        }
    }
