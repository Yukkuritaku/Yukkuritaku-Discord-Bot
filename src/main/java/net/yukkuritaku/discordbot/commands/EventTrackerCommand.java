...
public class EventTrackerCommand extends ListenerAdapter {

    @Override
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        String[] arguments = event.getMessage().getContentRaw().split("\\s+");
        if(arguments[0].equalsIgnoreCase(Main.Prefixcmd + "test")){
     ...
     * でも今は無理そう(´・ω・｀)
     */
    public void getRankingTrackers(GuildMessageReceivedEvent event){
        try {
